...
def RVEgen2Alya(path, num_caso, density, viscosity, volume_fraction, tipo_fallo, anchura, h_capa, L_pro, n_elementos_gap, n_elementos_towsingap,
                n_elementos_capa, n_capas, angulos_tows, n_tows45, Lset, ol, ajus_ol, ol_izd, ol_drch, AlyaSet, debug, queue, numCPUs):
...
# 	tipo_fallo = "O"
# 	anchura = 5.0
# 	h_capa = 0.182			# Altura (espesor) de la capa.
# 	L_pro = 0.2
...
# 	ol_drch = 0.6
	...
	if tipo_fallo=="N":
		...
		datos_input, n_nodos, n_espesor, Ldom = NoFallos(anchura, h_capa, L_pro, angulos_tows, n_capas, n_tows45, n_elementos_gap, n_elementos_towsingap, n_elementos_capa)
	elif tipo_fallo=="O":
		caseName = "Caso_" + str(num_caso)
		datos_input, n_nodos, n_espesor, Ldom = Overlap(anchura, h_capa, L_pro, ol, ajus_ol, ol_izd, ol_drch, angulos_tows, n_capas, n_tows45, n_elementos_gap, n_elementos_towsingap, n_elementos_capa)
	elif tipo_fallo=="G": 
		caseName = "Caso_" + str(num_caso)
		datos_input, n_nodos, n_espesor, Ldom = Gap(anchura, h_capa, L_pro, ol, ajus_ol, ol_izd, ol_drch, angulos_tows, n_capas, n_tows45, n_elementos_gap, n_elementos_towsingap, n_elementos_capa)
...
# 	inicio =time.time()
	...
	z0 = 0.0  # Primer valor
	zn = n_capas*h_capa  # Último valor
	...
	# Definir los valores iniciales y finales para discretización en Z
	z0c = z0+(n_capas*h_capa/(n_espesor-1))/2.0  # Primer valor
	znc = zn-(n_capas*h_capa/(n_espesor-1))/2.0  # Último valor
...
# 	inicio = time.time() 
	...
	del A, anchura, angulos_tows, archivo_stl, B, C, cent, centros_Yarn, comb
	...
	del dot_product, fin, i, inicio, j, k, L_pro, n, n_elementos_capa, n_elementos_gap
	...
	del n_espesorc, n_nodos, n_tows45, nc, node_tows, posicion_anterior, posicion_menor
...
# 	inicio = time.time()
	Elementsetmaterials = np.zeros((len(matriz_4dc[:,0,0,0]) * len(matriz_4dc[0,:,0,0]) * len(matriz_4dc[0,0,:,0]), int(2)))
...
# 	print('Getting Alya boundaries ...')
	...
	for j in range(len(numero_elemento[0,:,0])):
		for k in range(len(numero_elemento[0,0,:])):
			b_list1.append([int(posicion_n_nodo[0,j,k]), int(posicion_n_nodo[0,j,k+1]), int(posicion_n_nodo[0,j+1,k+1]), int(posicion_n_nodo[0,j+1,k]), int(numero_elemento[0,j,k])])
			nboun +=1
	b_list2 = []
	for j in range(len(numero_elemento[-1,:,0])):
		for k in range(len(numero_elemento[-1,0,:])):
			b_list2.append([int(posicion_n_nodo[-1,j,k]), int(posicion_n_nodo[-1,j+1,k]), int(posicion_n_nodo[-1,j+1,k+1]), int(posicion_n_nodo[-1,j,k+1]), int(numero_elemento[-1,j,k])])
			nboun +=1
	b_list3 = []
	for i in range(len(numero_elemento[:,0,0])):
		for k in range(len(numero_elemento[0,0,:])):
			b_list3.append([int(posicion_n_nodo[i,0,k]), int(posicion_n_nodo[i+1,0,k]), int(posicion_n_nodo[i+1,0,k+1]), int(posicion_n_nodo[i,0,k+1]), int(numero_elemento[i,0,k])])
			nboun +=1
	b_list4 = []
	for i in range(len(numero_elemento[:,-1,0])):
		for k in range(len(numero_elemento[-1,0,:])):
			b_list4.append([int(posicion_n_nodo[i+1,-1,k]), int(posicion_n_nodo[i,-1,k]), int(posicion_n_nodo[i,-1,k+1]), int(posicion_n_nodo[i+1,-1,k+1]), int(numero_elemento[i,-1,k])])
			nboun +=1
	b_list5 = []
	for i in range(len(numero_elemento[:,0,0])):
		for j in range(len(numero_elemento[0,:,0])):
			b_list5.append([int(posicion_n_nodo[i,j+1,0]), int(posicion_n_nodo[i+1,j+1,0]), int(posicion_n_nodo[i+1,j,0]), int(posicion_n_nodo[i,j,0]), int(numero_elemento[i,j,0])])
			nboun +=1
	b_list6 = []
	for i in range(len(numero_elemento[:,-1,0])):
		for j in range(len(numero_elemento[-1,:,0])):
			b_list6.append([int(posicion_n_nodo[i,j,-1]), int(posicion_n_nodo[i+1,j,-1]), int(posicion_n_nodo[i+1,j+1,-1]), int(posicion_n_nodo[i,j+1,-1]), int(numero_elemento[i,j,-1])])
			nboun +=1
...
# 	inicio = time.time()
	...
	if AlyaSet == 'All':
		...
		writeAlyaSet2(outputMeshPath, caseName, Lset, n_capas, nodes)
	fin = time.time()
	tiempo_ej = fin-inicio
...
#		gravity = Presion_de_inyeccion/(density*length[j])
		...
		# Alya Dom
		writeAlyaDom(path, fileName, icase, int(numerodenodos), numerodeelementos, numerodeboundelems, nmate, periodicityMethod, fieldFlag)
		# Alya Dat
