...
enqueue_compss_cmd="enqueue_compss --worker_working_dir=$PWD --project_name=$PROJECT_NAME --job_execution_dir=$EXECUTION_FOLDER --log_dir=$EXECUTION_FOLDER --qos=$QOS --exec_time=$EXEC_TIME --pythonpath=$PYTHONPATH --num_nodes=$NUM_NODES --worker_in_master_cpus=16 --keep_workingdir --network=ethernet --constraints=highmem "
