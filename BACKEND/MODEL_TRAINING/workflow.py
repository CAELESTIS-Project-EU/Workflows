...
def workflow(path, execution_folder, data_folder):
    with open(path) as f:
        ...
        model_creation(phases.get("sampler"), phases.get("training"), data.get("outputs"), data.get("problem"),
                       execution_folder, data.get("input"), phases.get("sim"), data_folder)
    return


def model_creation(samplerData, training, outputs, problem, execution_folder, input, simType, data_folder):
