...
launch_compss -d --sc_cfg=mn.cfg --master_node="$SLURMD_NODENAME" --worker_nodes="" --network=ethernet --output_profile=$EXECUTION_FOLDER/time --keep_workingdir --log_dir=$EXECUTION_FOLDER --worker_working_dir=$PWD --scheduler=es.bsc.compss.scheduler.orderstrict.fifo.FifoTS --job_execution_dir=$EXECUTION_FOLDER --pythonpath=$PYTHONPATH --worker_in_master_cpus=48 $INSTALL_DIR/WORKFLOWS/api.py $YAML $EXECUTION_FOLDER $DATA_DIR
