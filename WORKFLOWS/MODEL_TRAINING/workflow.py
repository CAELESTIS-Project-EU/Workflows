...
def workflow(path, execution_folder, data_folder):
    with open(path) as f:
        ...
        workflow_execution(phases.get("sampler"), data.get("problem"), execution_folder, data.get("input"),
                           phases.get("sim"), data.get("outputs"), data_folder, phases.get("training"))
    return


def workflow_execution(samplerData, problem, execution_folder, input_yaml, simType, outputs, data_folder, training):
